...
public class AbilitySelectUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    {
        Weapon = weapon;
        e_onClick.AddListener(() => GameManager.Instance.photonView.RPC("SetPlayerWeapon", RpcTarget.All, PhotonNetwork.LocalPlayer.NickName, Weapon._name));
    }
    ...
    public void SetValue(UpgradeSO upgrade, int curLevel = 0)
    {
        ...
        if (curLevel == 0)
            ...
            _desc.text += $"\n\n현재 레벨 : {curLevel}";
        e_onClick.AddListener(() => GameManager.Instance.photonView.RPC("SetPlayerUpgrade", RpcTarget.All, PhotonNetwork.LocalPlayer.NickName, Upgrade._name));
    }
